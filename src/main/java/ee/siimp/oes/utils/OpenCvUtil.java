...
public class OpenCvUtil {
    ...
    private static final int MAX_LETTER_WIDTH = 25;
    ...
    public static Mat createContourMask(Mat srcImage, MatVector contours, Mat debugImage) {
        ...
        for (int i = 0; i < contours.size(); i++) {
            try(Rect r = opencv_imgproc.boundingRect(contours.get(i))) {
                if (r.height() < MAX_LETTER_HEIGHT && r.height() > MIN_LETTER_HEIGHT && r.width() < MAX_LETTER_WIDTH && r.width() > MIN_LETTER_WIDTH) { 
